...
**ARC (Adaptive Replacement Cache) was scoped and partially implemented** on a separate branch (`arc`), but deferred from the main engine after review surfaced a correctness risk in one of its boundary conditions that needed more validation time than was available. Four simpler, fully-verified policies were prioritized over a fifth, more sophisticated one that couldn't yet be trusted.
